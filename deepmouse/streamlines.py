...
from deepmouse.maps.util import (
    get_child_ids,
    get_default_structure_tree,
    print_descriptions,
)
...
def laplace_solution(positions, is_outer, is_inner):
    ...
    for i, position in enumerate(positions):
        ...
        if not (is_outer[i] or is_inner[i]):
            ...
            A[i, i] = -len(
                ni
            )  # normally = -2*3, but we have some edges without boundary conditions
    ...
    print(
        "Laplace solution mean: {} residual norm: {}".format(
            np.mean(solution[0]), solution[3]
        )
    )
...
def save_inner_and_outer_edges():
    ...
    positions_12345 = np.concatenate(
        (positions["1"], positions["2/3"], positions["4"], positions["5"])
    )
...
def save_inner_outer_flags():
    ...
    for i in range(positions_all.shape[0]):
        if i % 1000 == 0:
            print(
                "Checking inner/outer edge {} of {}".format(i, positions_all.shape[0])
            )
        p = positions_all[i, :]
...
def get_interpolator(positions, depths):
    ...
    return Rbf(
        positions[:, 0], positions[:, 1], positions[:, 2], depths, epsilon=1
    )  # smaller edge effects than gaussian
...
def get_gradient(interpolator, position):
    ...
    """
    d = 0.1  # fraction voxel to move for finite difference
...
def get_streamline(interpolator, origin, step_size=0.1, to_surface=True):
    ...
    streamline = [p]
    c = 0
    while (to_surface and depth > 0.25) or (
        not to_surface and depth < 6.75
    ):  # unstable at the very edges
        c = c + 1
        if c > 500:
            print("stuck")
            print(streamline)
...
class CompositeInterpolator:
    ...
    def __init__(self, positions, depths):
        ...
        self.intervals[:, 0] = (
            self.intervals[:, 0] - buffer
        )  # extend each slice to reduce edge effects
        self.intervals[:, 1] = self.intervals[:, 1] + buffer
        print(self.intervals)
        self.interpolators = []
        for i in range(n):
            print("Setting up slice {} of {}".format(i, n))
            subset = get_slice_indices(
                positions, self.intervals[i, 0], self.intervals[i, 1]
            )
    ...
    def __call__(self, *args, **kwargs):
        ...
        for i in range(len(self.boundaries) - 1):
            if (
                self.boundaries[i] <= x <= self.boundaries[i + 1]
            ):  # have to include both edges
                # print('using ' + str(i))
...
if __name__ == "__main__":
    ...
    tsne = (
        TSNE()
    )  # note: this varies from run to run - check that it forms a single manifold
