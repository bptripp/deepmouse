...
class NormalizedFlatmap:
    def __init__(self, area):
        ...
        sd = (np.sum(squared_distances) / len(positions_2d)) ** 0.5
...
class Gaussian2D:
    ...
    def __str__(self):
        return "weight: {} mean: {} covariance: {}".format(
            self.weight, self.mean, self.covariance.flatten()
        )
    ...
    def approx(self):
        ...
        if weight_sum > 0:
            ...
            for g in self.gaussians:
                ...
                covariance = covariance + g.weight / weight_sum * (
                    between + g.covariance
                )
        else:
            mean = [0, 0]
            covariance = [[0, 0], [0, 0]]
...
def get_primary_gaussians(area):
    ...
    for position_3d in positions_3d:
        ...
        covariance = [
            [0, 0],
            [0, 0],
        ]  # primary voxels have no spread in primary voxel space
...
def propagate_gaussians_through_isocortex(
    gaussians, positions_3d, data_folder="data_files/", omit_experiment_rank=None
):
    """
    ...
    if omit_experiment_rank is not None:
        ...
        # print("***************")
        # print(cortex_weights_for_selected_source_voxels.shape, cortex_weights.shape, nodes.shape)
        # print(ranked_experiment_indices)
        cortex_weights, nodes = remove_experiment(
            cortex_weights, nodes, ranked_experiment_indices[omit_experiment_rank]
        )
        # print("after",cortex_weights_for_selected_source_voxels.shape, cortex_weights.shape, nodes.shape)
        # plt.plot(sums)
        # plt.show()
        # print("***************")
...
if __name__ == "__main__":
    ...
    # areas = ['AUDp', 'SSp-bfd', 'SSp-m', 'SSp-ul']
    # areas = ["SSp-n"]
    for area in areas:
        gaussians, positions_3d = get_primary_gaussians(area)
        for omit_experiment_rank in range(2, 4):
            propagated = propagate_gaussians_through_isocortex(
                gaussians, positions_3d, omit_experiment_rank=omit_experiment_rank
            )
            ...
            print("generated/propagated {} omit {}".format(area, omit_experiment_rank))
